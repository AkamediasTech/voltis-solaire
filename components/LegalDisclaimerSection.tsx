...
export const LegalDisclaimerSection: React.FC = () => {
  const disclaimers = [
    ...
    "(4) EDF solutions solaires se charge de vos démarches administratives (urbanisme, financement, demande de raccordement)",
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="space-y-2">
          {disclaimers.map((text, index) => (
            <p key={index} className="text-[10px] md:text-xs text-gray-500 leading-relaxed text-justify font-normal">
              {text}
            </p>
          ))}
        </div>
